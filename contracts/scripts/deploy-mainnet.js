...
async function main() {
  ...
  console.log("\n╔═══════════════════════════════════════════════╗");
  console.log("║   MoltSpace — Deploy to Monad MAINNET         ║");
  ...
  // Safety check
  if (deployerBalance < hre.ethers.parseEther("1")) {
    console.error("ERROR: Deployer balance too low. Need at least 1 MON for deployment gas.");
    process.exit(1);
  }
  ...
  // ═══════════════════════════════════════════
  const fundAmount = hre.ethers.parseEther("1"); // 1 MON for mainnet
  console.log("\n[Fund] Sending 1 MON to ExpeditionManager reward pool...");
...
# Auto-generated by deploy-mainnet script on ${new Date().toISOString()}
...
npx hardhat verify --network monad-mainnet ${expeditionAddress} "${mothershipAddress}"

npx hardhat verify --network monad-mainnet ${gameManagerAddress} "${nftAddress}" "${mothershipAddress}" "${expeditionAddress}" "${treasury}"
`);
